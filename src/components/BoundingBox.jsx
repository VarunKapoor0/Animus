...
export default function BoundingBox({ x, y, isScanning, objectType }) {
  ...
  // Glitch entry animation on mount / position change
  useEffect(() => {
  ...
  // Recognition beat: when scanning finishes and we have an object type,
  // briefly flash the identifying label
  useEffect(() => {
    if (prevScanning.current && !isScanning && objectType) {
      setShowIdentifying(true);
      const t = setTimeout(() => setShowIdentifying(false), 800);
  ...
  return (
    ...
    >
      ...
      {/* Recognition beat — brief flash of object name when scan completes */}
      {showIdentifying && objectType && (
        <div className="absolute -bottom-7 left-0 right-0 text-center">
          <span
            className="font-mono text-[9px] uppercase tracking-widest text-neon-cyan"
            style={{ textShadow: '0 0 8px #00f5ff', animation: 'flicker 0.15s infinite' }}
          >
            {objectType.length > 20 ? objectType.substring(0, 20) + '...' : objectType}
          </span>
        </div>
      )}
    </div>
